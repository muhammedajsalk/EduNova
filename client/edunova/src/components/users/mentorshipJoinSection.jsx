...
const MentorshipVideoCallSection = () => {
  ...
  const { id: mentorshipId, instructorId } = useParams();
  ...
  }, [mentorshipId, instructorId, myStream, sendStream, setRemoteAns]);
  ...
  const joinVideoCall = async () => {
    ...
    if (offerRef.current) {
      ...
      setRemoteUserId(instructorId)
      socket.emit("video-room", { mentorshipId, userId: user._id, role: "user" });
    }
    ...
    <div className="h-screen bg-slate-900 flex flex-col overflow-hidden">
      <div className="bg-slate-800 border-b border-slate-700 px-6 py-3 flex-shrink-0 flex justify-between items-center">
        ...
        <div className="flex items-center gap-4">
          <h2 className="text-lg font-semibold text-white">Mentorship Session</h2>
        ...
        {!isInCall ? (
          <div className="h-full flex items-center justify-center bg-gradient-to-br from-slate-900 to-slate-800">
            <div className="text-center">
              ...
              <h3 className="text-2xl font-semibold text-white mb-2">Ready to start your session?</h3>
              <p className="text-slate-400 mb-8">Make sure your camera and microphone are working.</p>
              <button
                onClick={joinVideoCall}
                className="bg-green-600 hover:bg-green-700 text-white font-semibold py-3 px-8 rounded-lg flex items-center gap-2"
              >
                <Phone className="w-5 h-5" /> Join Call
              </button>
            </div>
          </div>
        ) : (
          <div className="relative w-full h-full">
            <div className="w-full h-full flex items-center justify-center">
              <div className="relative w-full h-full max-w-5xl max-h-[80vh] bg-slate-800 rounded-lg overflow-hidden shadow-2xl">
...
export default React.memo(MentorshipVideoCallSection);